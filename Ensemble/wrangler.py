...
class Dataset:
    ...
    def __init__(self, df, name, targets=None):
        if targets is None:
...
class Wrangler:
    def __init__(self):
        ...
        for f in os.listdir('./data/'):
            ...
            elif f == "letter-recognition.data.txt":
                self.clean_letter_data("./data/" + f)
    ...
    def clean_iris_data(self):
        ...
        self.iris = Dataset(iris_data, "iris", iris_targets)

    def clean_letter_data(self, filename):
        column_names = ["targets", "x-box", "y-box", "width", "height", "onpix",
                        "x-bar", "y-bar", "x2-bar", "y2-bar", "xy-bar", "x2ybr",
                        "xy2br", "xedge", "xegvy", "y-ege", "yegvx"]

        data = pd.read_csv(filename, names=column_names)
        self.letter = Dataset(data, "letter")
        # print(self.letter.data.head())
        self.letter.data = self.normalize(self.letter.data)
        # print(self.letter.data.head())
    ...
    def clean_car_data(self, filename):
        ...
        self.car = Dataset(data, "car")
        # print(self.car.data.head())
        self.car.data = self.normalize(self.car.data)
        # print(self.car.data.head())
    ...
    def clean_diabetes_data(self, filename):
        ...
        self.diabetes = Dataset(data, "diabetes")
        self.diabetes.data = self.normalize(self.diabetes.data)
    ...
    def clean_mpg_data(self, filename):
        ...
        self.mpg = Dataset(data, "mpg")
        self.mpg.data = self.normalize(self.mpg.data)
